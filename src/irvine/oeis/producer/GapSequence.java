...
/**
 * Produce a sequence from a GAP program.
...
public class GapSequence extends AbstractSequence implements Closeable {
  ...
  public GapSequence(final String gapProgram) {
    ...
    mOut.println(gapProgram); // Send the program to GAP
    switch (programType) {
      case "list":
        break;
      default:
  ...
  public Z next() {
    if (mTerms.isEmpty()) {
      ...
      try {
        ...
        throw new UnsupportedOperationException("GAP failed to produce more terms", e);
      }
    }
    if (mTerms.isEmpty()) {
  ...
  public void close() {
    ...
    // but it gives GAP a chance to exit cleanly
    try {
