...
public class EulerTransform implements Sequence {
  ...
  private static final int ESTLEN = 128; // estimated length of arrays
  private final ArrayList<Z> mFs = new ArrayList<>(ESTLEN); // underlying sequence
  private final ArrayList<Z> mBs = new ArrayList<>(ESTLEN); // resulting sequence
  private final ArrayList<Z> mCs = new ArrayList<>(ESTLEN); // auxiliary sequence 
  ...
  public Z next() { 
    ...
    ++mN; // starts with 1
    final Z fNext = advance(); // underlying sequence will see mN = 1, 2, 3, ... 
    mFs.add(fNext == null ? Z.ZERO : fNext); // get next a(n)
