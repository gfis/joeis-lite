  ...
  public Z next() {
    while (true) {
      switch (mLevel) {
        ...
        case 1: // additive: continue with an algorithm using Maple.msolve
          ...
          */
          while (mGood.size() <= 0) {
            ...
            TreeSet<Z> acands = new TreeSet<Z>(QuadraticCongruence.solve(Z.ONE, Z.valueOf(mDist), Z.valueOf(-mIncr2), pow10p1.add(1))); // for A116170: msolve(a*(a+1)-2,10^m+1)
        ...
        case 2: // multiplicative, simlar msolve algorithm
          while (mGood.size() <= 0) {
            final Z pow10p1 = mPow10.multiply(10);
            TreeSet<Z> acands = new TreeSet<Z>(QuadraticCongruence.solve(Z.ONE, Z.ZERO, Z.ZERO, pow10p1.add(mDist))); // for A115553: msolve(a^2,10^m+9)
            for (final Iterator<Z> ita = acands.iterator(); ita.hasNext();) {
              final Z ta = ita.next();
