...
public final class SequenceFactory {
  ...
  /**
   * Compute the terms and display them in the proper formatting.
   * args commandline arguments
   * @param out stream where to write the output
   */
  public static void compute(final String[] args, final OutputStream out) throws IOException {
    ...
    try {
      ...
      try {
        ...
        if (seq instanceof DeadSequence) {
          out.write(("Sequence is \"dead\" in the OEIS.").getBytes(StandardCharsets.US_ASCII));
          out.write(LS);
          out.flush();
          return;
        }
      ...
      try {
        if (generated) {
          out.write(("Implementation limits exceeded, cannot generate further terms for " + seqId + "\n" + e.getMessage()).getBytes(StandardCharsets.US_ASCII));
        } else {
          out.write(("Sorry " + seqId + " is not yet implemented").getBytes(StandardCharsets.US_ASCII));
        }
        out.write(LS);
        out.flush();
      } catch(Exception exc) {
  ...
  public static void main(final String[] args) throws IOException {
    final OutputStream out = new BufferedOutputStream(new FileOutputStream(FileDescriptor.out));
    compute(args, out);
  }
