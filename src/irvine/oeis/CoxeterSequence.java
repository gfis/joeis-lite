...
public class CoxeterSequence extends GeneratingFunctionSequence {
   ...
   *  This corresponds with the Mathematica routine <em>coxG</em>
   ...
   */
  public CoxeterSequence(int pwr, int c1, int c2) {
  	super();
    mNum = new Z[pwr + 1];
    mDen = new Z[pwr + 1];
    mNum[pwr] = Z.ONE;
   ...
   */
  public CoxeterSequence(int pwr, int ngen) {
		this(pwr, triangular(ngen), 2 - ngen);
	}

  /** Returns a triangular number
   *  @param ngen number of generators
   *  @return binomial(ngen + 1, 2)
   */
  private static int triangular(int ngen) {
      int result = 0; 
