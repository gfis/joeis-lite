...
import irvina.util.array.LongDynamicBooleanArray;
/**
 * Properties of a one- or two-dimensional cellular automaton
 * as described in Stephen Wolfram's book "A new kind of science".
 * @author Georg Fischer
 */
public abstract class CellularAutomatonSequence implements Sequence {
  ...
  protected LongDynamicBooleanArray mRow
   ...
   */
  protected CellularAutomatonSequence(final int offset) {
    //mOffset = offset;
    mN = offset - 1;
    mK = Z.valueOf(mN);
  }
   ...
   */
  protected CellularAutomatonSequence(final int offset, final int k) {
    this(offset);
    mK = Z.valueOf(k);
  }
...
//  } // getNextProperty
  ...
  protected Z getNextWithProperty() {
    ...
    if (loopCheck == 0) {
      throw new IllegalArgumentException("More than 10^8 iterations in CellularAutomatonSequence.getNextWithProperty()");
    }
    return mK;
  } // getNextWithProperty
