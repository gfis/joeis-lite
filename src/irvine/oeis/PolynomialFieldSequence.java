...
public class PolynomialFieldSequence extends AbstractSequence {
  ...
  //  protected static final ThetaFunctions THETA = ThetaFunctions.SINGLETON;
  ...
  @FunctionalInterface
  public interface PolynomialFieldFunction<PolynomialFieldSequence, Integer, Polynomial> {
    /**
     * Apply the function.
     ...
     */
    Polynomial apply(PolynomialFieldSequence self, Integer n);
  }
  ...
  // Generated by ../joeis-lite/internal/fischer/reflect/gen_pfs.pl at 2025-02-28 21:20
  private final Stack<Polynomial<Q>> mStackPol = new Stack<>();
  
  public Polynomial<Q> pop() {
  	return mStackPol.pop();
  }

  public Polynomial<Q> push(Polynomial<Q> poly) {
  	mStackPol.push(poly);
  	return poly;
  }

  public Polynomial<Q> getA() {
  	return mA;
  }

  public Polynomial<Q> setA(Polynomial<Q> poly) {
    mA = poly;
  	return mA;
  }   
  ...
  private static final ArrayList<PolynomialFieldFunction<PolynomialFieldSequence, Integer, Polynomial<Q>>> sLambda = new ArrayList<>(64);
  private static void setPost1(final int ipost, final String key, final PolynomialFieldFunction<PolynomialFieldSequence, Integer, Polynomial<Q>> lambda) {
    sLambda.add(lambda);
  ...
  {
    int ix = 0;
    setPost1(++ix, "p"       , (self, m) -> self.push(self.getPoly(self.advance())));
    setPost1(++ix, "<"       , (self, m) -> self.push(RING.shift(self.pop(), self.advance()))); 
    setPost1(++ix, "A"       , (self, m) -> self.push(self.getA()));
    setPost1(++ix, "x"       , (self, m) -> self.push(RING.x()));   
    setPost1(++ix, "l"       , (self, m) -> self.push(RING.one())); 
    setPost1(++ix, "^"       , (self, m) -> self.push(RING.pow(self.pop(), self.advance(), m)));
    setPost1(++ix, "^/"      , (self, m) -> self.push(RING.pow(self.pop(), new Q(self.advance(), self.advance()), m)));
    setPost1(++ix, "sub"     , (self, m) -> self.push(RING.substitute(self.getA(), self.pop(), m)));                        
    setPost1(++ix, "+"       , (self, m) -> { Polynomial<Q> op2 = self.pop(); return self.push(RING.add(self.pop(), op2)); });
    setPost1(++ix, "-"       , (self, m) -> { Polynomial<Q> op2 = self.pop(); return self.push(RING.subtract(self.pop(), op2)); });
    setPost1(++ix, "*"       , (self, m) -> { Polynomial<Q> op2 = self.pop(); return self.push(RING.multiply(self.pop(), op2, m + 1)); });
    setPost1(++ix, "/"       , (self, m) -> { Polynomial<Q> op2 = self.pop(); return self.push(RING.series(self.pop(), op2, m)); });
    setPost1(++ix, "abs"     , (self, m) -> self.push(RING.abs(self.pop())));
    setPost1(++ix, "acosh"   , (self, m) -> self.push(RING.acosh(self.pop(), m)));
    setPost1(++ix, "asin"    , (self, m) -> self.push(RING.asin(self.pop(), m)));
    setPost1(++ix, "asinh"   , (self, m) -> self.push(RING.asinh(self.pop(), m)));
    setPost1(++ix, "atan"    , (self, m) -> self.push(RING.atan(self.pop(), m)));
    setPost1(++ix, "atanh"   , (self, m) -> self.push(RING.atanh(self.pop(), m)));
    setPost1(++ix, "cos"     , (self, m) -> self.push(RING.cos(self.pop(), m)));
    setPost1(++ix, "cosh"    , (self, m) -> self.push(RING.cosh(self.pop(), m)));
    setPost1(++ix, "cot"     , (self, m) -> self.push(RING.series(RING.one(), RING.tan(self.pop(), m), m)));
    setPost1(++ix, "coth"    , (self, m) -> self.push(RING.series(RING.one(), RING.tanh(self.pop(), m), m)));
    setPost1(++ix, "csc"     , (self, m) -> self.push(RING.series(RING.one(), RING.sin(self.pop(), m), m)));
    setPost1(++ix, "csch"    , (self, m) -> self.push(RING.series(RING.one(), RING.sinh(self.pop(), m), m)));
    setPost1(++ix, "dif"     , (self, m) -> self.push(RING.diff(self.pop())));
    setPost1(++ix, "eta"     , (self, m) -> self.push(RING.eta(self.pop(), m)));
    setPost1(++ix, "exp"     , (self, m) -> self.push(RING.exp(self.pop(), m)));
    setPost1(++ix, "int"     , (self, m) -> self.push(RING.integrate(self.pop()).truncate(m)));
    setPost1(++ix, "inv"     , (self, m) -> self.push(RING.inverse(self.pop())));
    setPost1(++ix, "lambNegW", (self, m) -> self.push(RING.negate(RING.lambertW(self.pop(), m))));
    setPost1(++ix, "lambertW", (self, m) -> self.push(RING.lambertW(self.pop(), m)));
    setPost1(++ix, "log"     , (self, m) -> self.push(RING.log(self.pop(), m)));
    setPost1(++ix, "neg"     , (self, m) -> self.push(RING.negate(self.pop())));
    setPost1(++ix, "rev"     , (self, m) -> self.push(RING.reversion(self.pop(), m)));
    setPost1(++ix, "sec"     , (self, m) -> self.push(RING.sec(self.pop(), m)));
    setPost1(++ix, "sech"    , (self, m) -> self.push(RING.sech(self.pop(), m)));
    setPost1(++ix, "sin"     , (self, m) -> self.push(RING.sin(self.pop(), m)));
    setPost1(++ix, "sinh"    , (self, m) -> self.push(RING.sinh(self.pop(), m)));
    setPost1(++ix, "sqrt"    , (self, m) -> self.push(RING.sqrt(self.pop(), m)));
    setPost1(++ix, "tan"     , (self, m) -> self.push(RING.tan(self.pop(), m)));
    setPost1(++ix, "tanh"    , (self, m) -> self.push(RING.tanh(self.pop(), m)));
  }
  ...
  public PolynomialFieldSequence(final int offset, final String polys, final String postfix,
    ...
    try {
      for (int k = 0; k < mPostStrings.length; ++k) {
        String postr = mPostStrings[k];
        final Integer ix = sFunctions.get(postr);
        if (ix != null) {
          mPostInts[mPostLen ++] = ix;
        } else { // special operands/operators
          int iy = sFunctions.get(postr.charAt(0));
          mPostInts[mPostLen ++] = iy;
          switch(iy) {
            case '1': // A - no parameter
              break;
            case '2': // sub - no parameter
              break;
            case '3': // p - append the polynomial's index
              mPostInts[mPostLen ++] = Integer.parseInt(postr.substring(1));
              break;
            case '4': // < - append the shift count
              mPostInts[mPostLen ++] = Integer.parseInt(postr.substring(1));
              break;
            case '5': // ^ - determine the (rational) exponent and append num (and optional den)
              final int slashPos = postr.indexOf('/');
              if (slashPos >= 0) {
                mPostInts[mPostLen ++] = Integer.parseInt(postr.substring(1, slashPos)); // numerator
                mPostInts[mPostLen ++] = Integer.parseInt(postr.substring(slashPos + 1)); // denominator
              } else {
                mPostInts[mPostLen ++] = Integer.parseInt(postr.substring(1)); // numerator only
              }
              break;
            default: // unknown
              break;
...
//          case "theta_3": //
//            mStack.set(top, THETA.theta3z(mStack.get(top), m)));
//            break;
//          case "theta_4":
//            mStack.set(top, THETA.theta4z(mStack.get(top), m)));
//            break;
