...
while (<>) { # read inputfile
    s/\s+\Z//; # chompr
    my $line = $_;
    my ($aseqno, $superclass, $name, $keyword, $range) = split(/\t/, $line);
    $name    =~ s{  +}{ }g;
    $name    =~ s{ of integers?}     { }g;
    $name    =~ s{ which }           { that }g;
    $name    =~ s{ a }               { the }g;
    $name    =~ s{ [Ss]ums? }        { sum }g;
    $name    =~ s{\A(Numbers? )([nk] )?(that are |that have |with |that can be written as )?(the )?}{};
    ...
    #----------------
        ...
        m{\A(not the )?sum of (quant_.. )?(\w+) (distinct )?(least_. )?(pow_\d+)( in (quant_.. )?(\w+)? way)?} ) {
        ($arenot , $mquant , $mult   , $distinct, $least  , $pow    , $wquant , $ways   ) = 
        ($1 || "", $2 || "", $3 || -1, $4 || "" , $5 || "", $6 || -1, $8 || "", $9 || -1);
        &normalize();
        if ($ways >= 2) {
            if ($arenot =~ m{not}) {
                $callcode = "sumlino";
            ...
            print join("\t", $aseqno, $callcode, 0, $pow, $least, $mult, $distinct
                , $superclass, $name) . "\n";
        } else { # with ways
            $callcode = "sumway";
            print join("\t", $aseqno, $callcode, 0, $pow, $least, $mult, $distinct
    ...
    #----------------
        ...
        #   1          2                                 3             4          5     6           7          8
        m{\A(ordered )?(different )?ways of writing n as (the )?sum of (quant_.. )(\w+) (distinct )?(least_. )?(pow_\d+)} ) {
    ...
    # quant_eq one representation as the sum of six least_0 pow_2. <- A295484	FiniteSequence	Numbers that have exactly one representation as a sum of six nonnegative squares.	nonn,fini,full,synth	1..5
        ...
        #   1           2                      3       4             5           6     7           8          9
        m{\A(quant_.. )?(\w+) representations? (as|of) (the )?sum of (quant_.. )?(\w+) (distinct )?(least_. )?(pow_\d+)} ) {
    ...
    # representable in quant_ge two ways as sum of four distinct nonvanishing pow_2. <- A259058	LinearRecurrence	Numbers that are representable in at least two ways as sums of four distinct nonvanishing squares.	nonn,easy,synth	0..40
        ...
        #                    1           2          3       4             5           6     7           8          9
        m{\Arepresentable in (quant_.. )?(\w+) ways (as|of) (the )?sum of (quant_.. )?(\w+) (distinct )?(least_. )?(pow_\d+)} ) {
        ($wquant,   $mquant , $mult   , $distinct, $least  , $pow    , $ways   ) = 
        ($1  || "", $5 || "", $6 || -1, $7 || "" , $8 || "", $9 || -1, $2 || -1);
        $callcode = "sumway";
        &normalize();
    ...
    # of ways of writing n as the sum of 5 nonnegative pow_3. <- A173679	null	Number of ways of writing n as a sum of 5 nonnegative cubes.	nonn,	0..10000
        ...
        #      1                                          2           3     4           5          6
        m{\Aof (ordered )?ways of writing n as the sum of (quant_.. )?(\w+) (distinct )?(least_. )?(pow_\d+)} ) {
    ...
    # A117535 Number of ways of writing n as a sum of powers of 3, each power being used at most 4 times.
        ...
        $callcode = "sumwab";
        &normalize();
        print     join("\t", $aseqno, $callcode, 0, $base, $mquant, $mult
                  , $superclass, $name) . "\n";
...
sub normalize { # bring all attributes into a normalized form suitable for column headers; digitize number words
    ...
    $base     = &norm_quant($base  );
    $mquant   = &norm_quant($mquant);
    $wquant   = &norm_quant($wquant);
    $xquant   = &norm_quant($xquant);
    $distinct = length($distinct) > 0 ? "dist" : "maybe =";
    if (        length($least) > 0) {
        $least  =~ s{\Anon\-?zero.*|\Apositi.*}{\>\= 1};
        $least  =~ s{\Anon\-?nega.*}           {\>\= 0};
    } else {
        $least  = ">= 0";
    }
