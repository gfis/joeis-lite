A058067	mul	0
# p^t_p(e)
# M.w. a(p^e) = p^t_p(e). - _David W. Wilson_, Aug 14 2005 [t_p(e) = Sum_{k>=0: e > A090622(k, p)} (e - A090622(k, p)) = p * Sum_{k = 1..e} max(0, k - A090622(e-k, p)). In particular, t_p(e) = p*e*(e+1)/2 for e <= p. - _Andrey Zabolotskiy_, Nov 09 2017 and Sep 29 2020]Number of polynomial functions from Z to Z/nZ.

A059479	mult	1
#--------> p.pow(3*e - (e % 2))
# M.w. a(p^e) = p^(3e - (e % 2)). - _Mitch Harris_, Jun 09 2005Number of 3 X 3 matrices with elements from {0,...,n-1} such that the middle element of each of the eight lines of three (rows, columns and diagonals) is the square (mod n) of the difference of the end elements.

A060839	mult	1
# p.equals(Z.THREE) ? Z.valueOf(e == 1 ? 1 : 3) : (isMod1(3, p, 2) ? Z.ONE : Z.THREE)
# M.w. a(3) = 1, a(3^e) = 3, e >= 2, a((3k+1)^e) = 3, a((3k+2)^e) = 1. _David W. Wilson_, May 22 2005Number of solutions to x^3 == 1 mod n.
...
# Sum_{n > 0} a(n)*x^n/(1 + x^n) = x. Moebius transform of A048298. M.w. a(2^e) = 2^(e - 1), a(p) = -1 for p > 2, a(p^e) = 0 for p > 2 and e > 1. - _Vladeta Jovovic_, Jan 02 2003Sum_{n >= 1} a(n)/n^s = 1/(Sum_{n >= 1} (-1)^(n + 1)/n^s).
...
A069256	mult	1
#----> p.equals(Z.TWO) ? Z.ONE.shiftLeft(4*e-3) : Z.ONE.shiftLeft(ZUtils.valuation(p.subtract(1).multiply(p.square().subtract(1)), Z.TWO))
...
A072028	mult	1
#----> (isMod1(4, p, 1) && p.add(2).isPrime() ? p.add(2) : (isMod1(4, p, 3) && p.subtract(2).isPrime() ? p.subtract(2) : p)).pow(e)
...
A072029	mult	1
#----> (isMod1(4, p, 3) && p.add(2).isPrime() ? p.add(2) : (isMod1(4, p, 1) && p.subtract(2).isPrime() ? p.subtract(2) : p)).pow(e)
...
A084091	mult	0
#----> Z.valueOf(p.equals(Z.TWO) ? (1 + (((e & 1) == 0) ? 1 : -1))/2 : (p.equals(Z.THREE) ? 0 : (isMod1(6, p, 1) ? 1 : (((e & 1) == 0) ? 1 : -1))))
...
A087694	mult	1
#----> p.equals(Z.THREE) ? Z.THREE.pow(e) : (isMod1(3, p, 1) ? p.subtract(1).multiply(e).multiply(p.pow(e-1)) : p.pow(e/2))
...
A087786	mult	1
#----> p.pow((2*e/3) * 2).add(Integers.SINGLETON.sum(0, (e-1)/3, i -> (p.subtract(1).multiply(p.pow(e + i -1).multiply(p.equals(Z.THREE) && 3*i+1 == e ? 3 : 1)))))
...
A109386	mult	1
#----> p.equals(Z.TWO) ? Z.ONE.shiftLeft(e+1).subtract(1) : (p.pow(e+2).multiply(e+1).subtract(p.pow(e+1).multiply(e+2).add(1))).divide(p.subtract(1).square())
# M.w. a(2^e) = 2^(e+1)-1 and a(p^e) = (p^(e+2)*(e+1)-p^(e+1)*(e+2)+1)/(p-1)^2 for p>2. - _Vladeta Jovovic_, Jul 05 2005G.f. is the logarithm of the g.f. of A107742: Sum_{n>=1} (a(n)/n)*x^n = Log( Sum_{n>=0} A107742(n)*x^n ).
...
# M.w. a(p^e) = p^e if p = 2; (e+1)p^e if p == 1 (mod 4); (1+(-1)^e)/2 p^e if p == 3 (mod 4).a(n) = n times number of divisors of n of form 4m+1 - n times number of divisors of form 4m+3.
...
# Totally M.w. a(p) = F(p). - _Franklin T. Adams-Watters_, Jun 05 2006Replace each prime p in prime-factorization of n with p-th Fibonacci number.
...
# a(n) is M.w. a(2^e) = -1 if e>0, a(5^e) = 1, a(p^e) = (p^(e+1)-1)/(p-1) if p == 1, 9 (mod 10), a(p^e) = ((-p)^(e+1)-1)/(-p-1) if p == 3, 7 (mod 10).Expansion of (25phi(q)phi^3(q^5)-phi^5(q)/phi(q^5)-24)/40 in powers of q where phi(q) is a Ramanujan theta function.
...
# a(n) is M.w. a(3^e) = 1, a(2^e) = 2^(e+1) - 3, a(p^e) = (p^(e+1) - 1) / (p - 1) if p > 3.One quarter of the number of solutions to a^2 + b^2 + 3*c^2 + 3*d^2 = n.

A113652	mul	1
# p.equals(Z.TWO) ? Z.valueOf(-1 if e>0, Z.valueOf(e+1 if p == 1 (mod 4), (1 + (((e & 1) == 0) ? 1 : -1))/2 if p == 3 (mod 4)
# a(n) is M.w. a(2^e) = -1 if e>0, a(p^e) = e+1 if p == 1 (mod 4), (1 + (-1)^e)/2 if p == 3 (mod 4).Expansion of (1 - theta_4(q)^2) / 4 in powers of q.
...
# a(n) is M.w. a(p)=phi(p), a(p^k)=phi(p^k)-phi(p^(k-1)) and phi(n)=A000010(n).Number of complex, weakly primitive Dirichlet characters modulo n.
...
A115364	mult	1
#----> Z.valueOf(p.equals(Z.TWO) ? (e+1) * (e+2) / 2 : 1)
...
# a(n) is M.w. a(5^e) = (-1)^e, a(7^e) = 1, a(p^e) = a(p) * a(p^(e-1)) - p * a(p^(e-2)) otherwise.Coefficients of L-series for elliptic curve "35a3": y^2 + y = x^3 + x^2 - x.

A116073	mult	1
#--------> p.equals(Z.FIVE) ? Z.ONE : p.pow(e+1).subtract(1).divide(p.subtract(1))
# a(n) is M.w. a(5^e) = 1, a(p^e) = (p^(e+1)-1)/(p-1) otherwise.Sum of the divisors of n that are not divisible by 5.
...
# a(n) = b(12*n + 1) where b() is M.w. b(2^e) = b(3^e) = 0^e, b(p^e) = (1 + (-1)^e) / 2 * p^(2*e) if p == 7 or 11 (mod 12), b(p^e) = b(p) * b(p^(e-1)) - p^4 * b(p^(e-2)) if p == 1 or 5 (mod 12). - _Michael Somos_, Jun 24 2013Expansion of f(-q)^2 * Q(q) in powers of q.

A123331	mul	1
# p.equals(Z.TWO) ? Z.valueOf((3-(((e & 1) == 0) ? 1 : -1))/2, a(3^e) = 1, Z.valueOf(e+1 if p == 1 (mod 6), Z.valueOf((1+(((e & 1) == 0) ? 1 : -1))/2 if p == 5 (mod 6)
# a(n) is M.w. a(2^e) = (3-(-1)^e)/2, a(3^e) = 1, a(p^e) = e+1 if p == 1 (mod 6), a(p^e) = (1+(-1)^e)/2 if p == 5 (mod 6).Expansion of (c(q)^2/(3c(q^2))-1)/2 in powers of q where c(q) is a cubic AGM function.
...
A125096	mult	1
#--------> p.equals(Z.TWO) ? Z.valueOf(e == 1 ? 0 : 2) : (Z.valueOf(isMod2(8, p, 1, 3) ? e + 1 : (1 + (((e & 1) == 0) ? 1 : -1))/2))
...
A126246	mult	1
#----> p.equals(Z.TWO) && e == 1 ? Z.TWO : p.multiply(p.subtract(1).pow(e-1))
...
# a(n) is M.w. a(2^e) = 2 + ((-4)^(e+1) - 1)/5, a(3^e) = 1, a(p^e) = (q^(e+1) - 1) / (q - 1) where q = p^2 * Kronecker(-3, p) if p > 3.Expansion of 1 - (1/3) * b(q) * b(q^2) * c(q)^2 / c(q^2) in powers of q where b(), c() are cubic AGM functions.

A132004	mul	1
# p.equals(Z.TWO) ? Z.valueOf(2*0^e - 1, a(3^e) = 1, Z.valueOf(e + 1 if p == 1 (mod 4), Z.valueOf((1 + (((e & 1) == 0) ? 1 : -1)) / 2 if p == 3 (mod 4)
# a(n) is M.w. a(2^e) = 2*0^e - 1, a(3^e) = 1, a(p^e) = e + 1 if p == 1 (mod 4), a(p^e) = (1 + (-1)^e) / 2 if p == 3 (mod 4).Expansion of (1 - phi(q^3) / phi(q) * phi(-q^2) * phi(-q^6)) / 2 in powers of q where phi() is a Ramanujan theta function.
...
# a(n) is M.w. a(2^e) = -1 if e>0, a(p^e) = (1 + (-1)^e) / 2 if p == 5, 7 (mod 8), a(p^e) = e + 1 if p == 1, 3 (mod 8).Expansion of (1 - phi(-q) * phi(q^2)) / 2 in powers of q where phi() is a Ramanujan theta function.
...
A134015	mult	1
#--------> p.equals(Z.TWO) ? Z.valueOf(e == 1 ? 0 : -2) : (Z.valueOf(isMod1(4, p, 1) ? e+1 : (1+(((e & 1) == 0) ? 1 : -1))/2))
...
# a(n) is M.w. a(2^e) = -1 unless e=0, a(3^e) = 1, a(p^e) = e + 1 if p == 1 (mod 6), a(p^e) = (1 + (-1)^e) / 2 if p == 5 (mod 6).Expansion of (1 - psi(-q)^3 / psi(-q^3)) / 3 in powers of q where psi() is a Ramanujan theta function.
...
# a(n) is M.w. a(2^e) = -1 if e>0, a(3^e) = 2 - (-1)^e, a(p^e) = e+1 if p == 1, 5 (mod 12), a(p^e) = (1 + (-1)^e) / 2 if p == 7, 11 (mod 12).Expansion of 1 - eta(q) * eta(q^3) * eta(q^4)^3 / (eta(q^2)^2 * eta(q^12)) in powers of q.

A138805	mul	0
# b(3^e) = 3 if e>1, b(p^e) = e+1 if p == 1 (mod 6), b(p^e) = (1 + (((e & 1) == 0) ? 1 : -1)) / 2 if p == 5 (mod 6)
# a(n) = 2*b(n) where b() is M.w. b(3^e) = 3 if e>1, b(p^e) = e+1 if p == 1 (mod 6), b(p^e) = (1 + (-1)^e) / 2 if p == 5 (mod 6).Theta series of quadratic form x^2 + x*y + 7*y^2.
...
# a(n) is M.w. a(2^e) = 1, a(3^e) = -1 + 2 * (-1)^e, a(p^e) = e+1 if p == 1, 5 (mod 12), a(p^e) = (1 + (-1)^e) / 2 if p == 7, 11 (mod 12).Expansion of (2 - 3 * phi(q^3)^2 + phi(q)^2) / 4 in powers of q where phi() is a Ramanujan theta function.
...
A140782	mult	1
#----> p.pow(e+1).subtract(1).divide(p.subtract(1)).multiply(Z.valueOf(LongUtils.kronecker(13, p.longValue())).pow(e))
...
# M.w. a(p^e) = p^e if p == 1 (mod 4); ceiling(p^(e+1)/(p+1)) if p == 3 (mod 4); 2^(e-1) + 1 if p = 2. - _Jianing Song_, Apr 20 2019Number of squared hypotenuses mod n in two dimensions.
...
# M.w. a(4) = 1, a(8) = 2, a(16) = 4, a(2^e) = 0 for e = 1 or e >= 5; for odd primes p, a(p) = 3 if p == 1 (mod 4) and 1 if p == 3 (mod 4), a(p^e) = 0 for e >= 2. - _Jianing Song_, Mar 02 2019Number of quartic primitive Dirichlet characters modulo n.
...
# a(n) is M.w. a(2^e) = 2^(e+1) - 1, a(3^e) = -2 if e>0, a(p^e) = 1 if p>3. - _Michael Somos_, Feb 14 2011L.g.f.: Sum_{n>=1} a(n)*x^n/n = log(S(x)/x) where S(x) is the g.f. of Stern:s diatomic series (A002487).

A163746	mul	1
# p.equals(Z.TWO) ? Z.ONE, a(3^e) = 2-(((e & 1) == 0) ? 1 : -1), Z.valueOf(e+1 if p == 1 (mod 4), = (1-(((e & 1) == 0) ? 1 : -1))/2 if p == 3 (mod 4)
# a(n) is M.w. a(2^e) = 1, a(3^e) = 2-(-1)^e, a(p^e) = e+1 if p == 1 (mod 4), a(p^e) == (1-(-1)^e)/2 if p == 3 (mod 4).Expansion of (theta_3(q)^2 + 3 * theta_3(q^3)^2) / 4 - 1 in powers of q.
...
# a(n) is M.w. a(2^e) = a(5^e) = (-1)^e, a(p^e) = (1 + (-1)^e)/2 if p == 3, 17, 21, 27, 29, 31, 33, 39 (mod 40), a(p^e) = e+1 if p == 1, 9, 11, 19 (mod 40), a(p^e) = (-1)^e * (e+1) if p == 7, 13, 23, 37 (mod 40).Expansion of (phi(q) * phi(q^10) - phi(q^2) * phi(q^5)) / 2 in powers of q where phi() is a Ramanujan theta function.
...
A166699	mul	1
# Z.valueOf((a(p-1)+2)^e
# M.w. a(p^e) = (a(p-1)+2)^e. If n = Product p(k)^e(k) then a(n) = Product (a(p(k)-1)+2)^e(k).Totally multiplicative sequence with a(p) = a(p-1) + 2 for prime p.

A166700	mul	1
# Z.valueOf((a(p-1)+3)^e
# M.w. a(p^e) = (a(p-1)+3)^e. If n = Product p(k)^e(k) then a(n) = Product (a(p(k)-1)+3)^e(k).Totally multiplicative sequence with a(p) = a(p-1) + 3 for prime p.

A166701	mul	1
# Z.valueOf((a(p-1)+4)^e
# M.w. a(p^e) = (a(p-1)+4)^e. If n = Product p(k)^e(k) then a(n) = Product (a(p(k)-1)+4)^e(k).Totally multiplicative sequence with a(p) = a(p-1) + 4 for prime p.

A166702	mul	1
# Z.valueOf((a(p-1)+5)^e
# M.w. a(p^e) = (a(p-1)+5)^e. If n = Product p(k)^e(k) then a(n) = Product (a(p(k)-1)+5)^e(k).Totally multiplicative sequence with a(p) = a(p-1) + 5 for prime p.

A166703	mul	1
# Z.valueOf((a(p-1)+6)^e
# M.w. a(p^e) = (a(p-1)+6)^e. If n = Product p(k)^e(k) then a(n) = Product (a(p(k)-1)+6)^e(k).Totally multiplicative sequence with a(p) = a(p-1) + 6 for prime p.

A166704	mul	1
# Z.valueOf((a(p-1)+7)^e
# M.w. a(p^e) = (a(p-1)+7)^e. If n = Product p(k)^e(k) then a(n) = Product (a(p(k)-1)+7)^e(k).Totally multiplicative sequence with a(p) = a(p-1) + 7 for prime p.

A166705	mul	1
# Z.valueOf((a(p-1)+8)^e
# M.w. a(p^e) = (a(p-1)+8)^e. If n = Product p(k)^e(k) then a(n) = Product (a(p(k)-1)+8)^e(k).Totally multiplicative sequence with a(p) = a(p-1) + 8 for prime p.

A166706	mul	1
# Z.valueOf((a(p-1)+9)^e
# M.w. a(p^e) = (a(p-1)+9)^e. If n = Product p(k)^e(k) then a(n) = Product (a(p(k)-1)+9)^e(k).Totally multiplicative sequence with a(p) = a(p-1) + 9 for prime p.

A166707	mul	1
# Z.valueOf((a(p-1)+10)^e
# M.w. a(p^e) = (a(p-1)+10)^e. If n = Product p(k)^e(k) then a(n) = Product (a(p(k)-1)+10)^e(k).Totally multiplicative sequence with a(p) = a(p-1) + 10 for prime p.
...
# Completely M.w. a(2) = -1, a(5) = 0, a(p) = 1 if p == 1, 3, 7, 9 (mod 20), a(p) = -1 if p == 11, 13, 17, 19 (mod 20). - _Jianing Song_, Dec 30 2018a(n) = Kronecker Symbol (-5/n), n >= 0.

A226333	mul	1
# p.pow(3*e) if p=5, else p.pow(3*(e+1)).subtract(1).divide(p^3 - 1)
# a(n) is M.w. a(p^e) = p^(3*e) if p=5, else a(p^e) = (p^(3*(e+1)) - 1) / (p^3 - 1).Expansion of (E_4(q) - E_4(q^5)) / 240 in powers of q where E_4 is an Eisenstein series.
...
# M.w. a(3^e)=3^e, a(p^e) = p^(e-1)*(p.subtract(1) if p== 1 (mod 3) and a(p^e) = p^(e-1)*(p+1) if p = 2 (mod 3). - _R. J. Mathar_, Jul 10 2013The twisted Euler phi-function for the non-principal Dirichlet character mod 3.
...
# a(2) = 8, p.equals(Z.TWO) ? Z.valueOf(0 if e > 1, Z.valueOf(a(p) * a(p.pow(e-1)) - p^5 * a(p.pow(e-2)) if p > 2a(n) is M.w. a(2) = 8, a(2^e) = 0 if e > 1, a(p^e) = a(p) * a(p^(e-1)) - p^5 * a(p^(e-2)) if p > 2.Expansion of q * f(-q^2)^12 + 8 * q^2 * f(-q^4)^12 in powers of q where f() is a Ramanujan theta function.
...
# M.w. a(2^e) = 1 + (-1)^e, a(p^e) = p^floor(e/2) for odd prime p.Number of solutions to x^2 == n (mod 2*n) for 0 <= x < 2*n.
...
A255647	multp1	0
#----> p.equals(Z.TWO) || p.equals(Z.valueOf(11)) ? Z.valueOf(LongUtils.kronecker(-22, p.intValue()) == +1 ? e + 1 : (1 + (((e & 1) == 0) ? 1 : -1))/2)
# a(n) is M.w. a(p^e) = 1 if p = 2 or 11, a(p^e) = e + 1 if Kronecker(-22, p) = +1, a(p^e) = (1 + (-1)^e)/2 if Kronecker(-22, p) = -1, and with a(0) = 1.

A255648	mult	1
#----> p.equals(Z.TWO) ? Z.ONE : (p.equals(Z.THREE) ? Z.valueOf(e == 0 ? 1 : 2) : Z.valueOf(isMod1(6, p, 1) ? e + 1 : (1 + (((e & 1) == 0) ? 1 : -1))/2))
# a(n) is M.w. a(2^e) = 1, a(3^e) = 2 if e>1, a(p^e) = e+1 if p == 1 (mod 6), a(p^e) = (1 + (-1)^e) / 2 if p == 5 (mod 6).

A256678	mul	1
# a(p)???  p.equals(Z.TWO) ? Z.ONE : (p.equals(Z.valueOf(17)) ? Z.valueOf((((e & 1) == 0) ? 1 : -1)) : p.pow(e))
...
A257403	mult	1
#----> p.equals(Z.TWO) ? Z.valueOf(e == 1 ? 1 : 0) : (p.equals(Z.THREE) ? Z.ZERO : Z.valueOf(isMod2(8, p, 1, 3) ? e+1 : (1 + (((e & 1) == 0) ? 1 : -1)) / 2))
# M.w. a(2) = 1, a(2^e) = 0 if e>1, a(3^e) = 0^e, a(p^e) = e+1 if p == 1, 3 (mod 8), a(p^e) = (1 + (-1)^e) / 2 if p == 5, 7 (mod 8).

A257477	mult	1
#----> p.equals(Z.TWO) ? Z.valueOf(e == 1 ? 0 : (e == 2 ? -1 : 0)) : (p.equals(Z.THREE) ? Z.valueOf(e == 1 ? -1 : 0) : Z.valueOf((isMod2(8, p, 1, 3) ? 1 : (((e & 1) == 0) ? 1 : -1))))
...
A257900	mult	1
#---->p.equals(Z.TWO) ? Z.NEG_ONE : (p.equals(Z.THREE) ? Z.valueOf(1 + (((e & 1) == 0) ? 1 : -1)) : (isMod2(4, p, 3, 4) ? Z.valueOf((1 + (((e & 1) == 0) ? 1 : -1))/2) : Z.valueOf(e+1)))
...
A138507	mult	1
#--------> p.equals(Z.TWO) ? Z.TWO.negate().pow(e+1).subtract(1).divide(3) : (p.equals(Z.FIVE) ? Z.ONE : (isMod2(10, p, 3, 7) ? p.negate().pow(e+1).subtract(1).divide(p.add(1).negate()) : p.pow(e+1).subtract(1).divide(p.subtract(1))))
# a(n) is M.w. a(2^e) = ((-2)^(e+1) - 1) / 3, a(p^e) = ((-p)^(e+1) - 1) / (-p - 1) if p == 3, 7 (mod 10), a(p^e) = (p^(e+1) - 1) / (p - 1) if p == 1, 9 (mod 10).

A258260	multp1	0
#--------> p.equals(Z.TWO) ? (e == 1 ? Z.ONE : Z.ZERO) : (p.equals(Z.THREE) ? Z.valueOf(e == 1 ? -1 : 4*(((e & 1) == 0) ? 1 : -1)) : (p.mod(Z.FOUR).equals(Z.ONE) ? Z.ONE : Z.valueOf(((e & 1) == 0) ? 1 : -1)))
# a(n) is M.w. a(2) = 1, a(2^e) = 0 if e>1, a(3) = -1, a(3^e) = 4 * (-1)^e if e>1, a(p^e) = 1 if p == 1 (mod 4), a(p^e) = (-1)^e if p == 3 (mod 4).

A258998	multp1	0
#--------> p.equals(Z.TWO) ? (((e & 1) == 0) ? Z.valueOf(((e & 2) == 0) ? 1 : -1) : Z.ZERO) : (((e & 1) == 0) ? Z.ONE : Z.ZERO)
...
A138505	mult	1
#----> p.equals(Z.TWO) ? Z.NEG_ONE : (isMod1(4, p, 1) ? p.square().pow(e+1).subtract(1).divide(p.square().subtract(1)) : p.square().negate().pow(e+1).subtract(1).divide(p.square().negate().subtract(1)))
...
A259030	mult	1
#----> p.equals(Z.TWO) ? Z.valueOf(-(1 - (((e & 1) == 0) ? 1 : -1)) / 2) : Z.valueOf(LongUtils.kronecker(5, p.intValue())).pow(e)
# a(n) is M.w. a(2^e) = -(1 - (-1)^e) / 2 if e > 0, a(p^e) = Kronecker(5, p)^e if p > 2.

A259445	mult	1
#----> p.equals(Z.TWO) ? Z.TWO : p.pow(e)
...
# a(n) is M.w. a(2^e) = (-1)^e, a(3^e) = -1 if e>0, a(p^e) = e+1 if p == 1 (mod 6), a(p^e) = (1 + (-1)^e)/2 if p == 5 (mod 6).Expansion of (a(q) - a(q^2) - 2*a(q^3) + 2*a(q^6)) / 6 in powers of q where a() is a cubic AGM function.
...
# a(n) is M.w. a(2^e) = (-1)^e, a(3^e) = -2*(-1)^e if e>0, a(p^e) = e+1 if p == 1, 7 (mod 24), a(p^e) = (e+1) * (-1)^e if p == 5, 11 (mod 24), a(p^e) = (1 + (-1)^e) / 2 if p == 13, 17, 19, 23 (mod 24).a(n) is multiplicative with a(2^e) = (-1)^e, a(3^e) = -2*(-1)^e if e>0, a(p^e) = e+1 if p == 1, 7 (mod 24), a(p^e) = (e+1) * (-1)^e if p == 5, 11 (mod 24), a(p^e) = (1 + (-1)^e) / 2 if p == 13, 17, 19, 23 (mod 24).
...
# M.w. a(4) = 1, a(8) = 2, a(2^e) = 0 for e = 1 or e >= 4; a(3) = 1, a(9) = 4, a(3^e) = 0 for e >= 3; a(p) = 5 if p == 1 (mod 6) and 1 if p == 5 (mod 6), a(p^e) = 0 if p > 3 and e >= 2.Number of sextic primitive Dirichlet characters modulo n.
...
# M.w. a(p^e) = 2^A000045(e).Multiplicative with a(p^e) = 2^A000045(e).

A318474	mul	1
# Z.valueOf(2^A000045(e+1)
# M.w. a(p^e) = 2^A000045(e+1).Multiplicative with a(p^e) = 2^A000045(e+1).
...
# Numerators of the sequence whose Dirichlet convolution with itself yields A318307, which is M.w. A318307(p^e) = 2^A002487(e).Numerators of the sequence whose Dirichlet convolution with itself yields A318307, which is multiplicative with A318307(p^e) = 2^A002487(e).
...
# M.w. a(2) = 1, a(4) = 2, a(2^e) = 4 for e >= 3; a(3) = 2, a(3^e) = 6 if e >= 2; for other primes p, a(p^e) = 6 if p == 1 (mod 6), a(p^e) = 2 if p == 5 (mod 6).Number of solutions to x^6 == 1 (mod n).

A319101	mul	1
# a(7) = 1, a(7^e) = 7 if e >= 2;
# M.w. a(7) = 1, a(7^e) = 7 if e >= 2; for other primes p, a(p^e) = 7 if p == 1 (mod 7), a(p^e) = 1 otherwise.Number of solutions to x^7 == 1 (mod n).

A319442	mul	1
# a(3^e) = 2*e + 1, Z.valueOf((e + 1).square() if p == 1 (mod 3) : e + 1 if p == 2 (mod 3)
# M.w. a(3^e) = 2*e + 1, a(p^e) = (e + 1)^2 if p == 1 (mod 3) and e + 1 if p == 2 (mod 3).Number of divisors of n over the Eisenstein integers.
...
# M.w. a(p^e) = Sum_{d|e, gcd(d, e/d)==1} p^d.a(1) = 1; a(n) = sum of exponential unitary divisors of n for n > 1.
...
# M.w. a(3^e) = 3^e, a(p^e) = (p^(e+1) - 1)/(p - 1) if p == 2 (mod 3), and (p^(e+1) + (-1)^e)/(p + 1) if p == 1 (mod 3). - _Amiram Eldar_, Oct 25 2020Expansion of Sum_{k>=1} k * x^k / (1 + x^k + x^(2*k)).

A326575	mul	1
# p^e if p < 5, (p.pow(e+1)-(-1)^(e+1)).divide(p+1) if p == 5 (mod 6), : (p.pow(e+1).subtract(1).divide(p.subtract(1)) if p == 1 (mod 6)
# M.w. a(p^e) = p^e if p < 5, (p^(e+1)-(-1)^(e+1))/(p+1) if p == 5 (mod 6), and (p^(e+1)-1)/(p-1) if p == 1 (mod 6). - _Amiram Eldar_, Dec 02 2020Expansion of Sum_{k>=1} k * x^k * (1 + x^(2*k)) / (1 + x^(2*k) + x^(4*k)).

A328271	mul	1
# Z.valueOf(Sum_{i=0
# M.w. a(p^e) = Sum_{i=0..floor(e/2)} p^(2*e-4*i) for prime p, i.e., a(p^(2*e)) = (p^(4*e+4).subtract(1).divide(p^4-1) and a(p^(2*e+1)) = p^2 * (p^(4*e+4).subtract(1).divide(p^4-1) for prime p. - _Werner Schulte_, Jul 24 2021Expansion of Sum_{k>=1} x^(k^2) * (1 + x^(k^2)) / (1 - x^(k^2))^3.
...
# M.w. a(p^e) = p^e if p = 2 or e is a multiple of p, otherwise a(p^e) = p^((p*floor(e/p)) + (2e mod p)).Multiplicative with a(p^e) = p^e if p = 2 or e is a multiple of p, otherwise a(p^e) = p^((p*floor(e/p)) + (2e mod p)).
...
A344879	mul	1
#cnot this seq.p.equals(Z.TWO) ? Z.ONE.shiftLeft(1+e).subtract(1) : p.pow(e).subtract(1)
