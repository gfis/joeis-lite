...
my $line = "";
my ($aseqno, $callcode, $offset, $seqtype, $prefix, $period, @rest);

while (<>) {
    s/\s+\Z//; # chompr
    $line = $_;
    next if $line !~ m{\AA\d+}; # skip lines that do not start with an A-number
    ($aseqno, $callcode, $offset, $seqtype, $prefix, $period, @rest) = split(/\t/, $line);
    if ($prefix !~ m{\?}) {
        ...
        print join("\t", $aseqno, $callcode, $offset, $seqtype, $prefix, $period, @rest) . "\n";
    } else {
    	print STDERR "$line\n";
    }
} # while <>
